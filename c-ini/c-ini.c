...
parse_basic_data_type(struct parser* p, enum token tok, enum c_data_type* type)
{
    ...
    if (cstr_equal("struct", p->value.str))
    {
        ...
        else if (cstr_equal("strlist", p->value.str))
        {
            if (scan_next(p) != '*')
                return parser_error(
                    p,
                    "Built-in string type 'strlist' must be of type 'struct "
                    "strlist*'\n");
            *type = CDT_STRLIST_CUSTOM;
            return scan_next(p);
        }
    }
...
static void gen_source_ini_parser(struct mstream* ms)
{
    ...
    mstream_cstr(
        ms,
        "struct strview strview(int off, int len)\n"
        "{\n"
        "    struct strview sv;\n"
        "    sv.off = off;\n"
        "    sv.len = len;\n"
    ...
    mstream_cstr(
        ms,
        "static float strview_to_float(struct strview str, const char* "
        "data)\n"
        "{\n"
        "    int   begin = str.off;\n"
    ...
    mstream_cstr(
        ms,
        "static char* strview_dup_cstr(struct strview str, const char* "
        "data)\n"
        "{\n"
...
static void gen_source_fwrite(struct mstream* ms, const struct section* section)
{
    ...
    mstream_fmt(
        ...
        section->struct_name,
        section->struct_name);
    mstream_cstr(ms, "    char** list; (void)list;\n");
    ...
    struct mstream* ms, const struct section* section, const struct key* key)
{
    ...
    switch (key->type)
    {
        ...
        case CDT_STR_DYNAMIC:
            ...
            mstream_fmt(
                ms,
                "    s->%S = strview_dup_cstr(p->value.string, p->source);\n"
                "    if (s->%S == NULL)\n"
